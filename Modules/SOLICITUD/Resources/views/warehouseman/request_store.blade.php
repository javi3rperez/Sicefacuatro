...
<table>
  ...
  <tbody>
    @foreach($solicitud->items as $item)
      <tr>
        ...
        <td>{{ $solicitud->observation ?? '---' }}</td> {{-- ✅ Ahora muestra la observación general --}}
      </tr>
    @endforeach
  </tbody>
</table>
  ...
  <div class="signature-line">
    ...
    <input type="text" name="signature_position"
      value="{{ old('signature_position', $solicitud->signature_position ?? '') }}"
      style="border:none; border-bottom:1px solid #000; width:300px;">
  </div>
  ...
  <div class="actions">
    {{-- Aprobar --}}
    <form action="{{ route('solicitud.admin.request.updateStatus', $solicitud->id) }}" method="POST" style="display:inline;">
      @csrf
      @method('PUT')
